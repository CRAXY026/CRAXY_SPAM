...
import git
import heroku3
# Changed root to CRAXYSPAM
from CRAXYSPAM import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9
from CRAXYSPAM import OWNER_ID, SUDO_USERS, HEROKU_APP_NAME, HEROKU_API_KEY, CRAXYversion
from CRAXYSPAM import CMD_HNDLR as hl
from telethon.tl.functions.users import GetFullUserRequest
# alive Pic By Default It's Will Show Our
from CRAXYSPAM import ALIVE_PIC
from telethon import events, version, Button
from telethon.tl.custom import button
from time import time
from datetime import datetime

DEAD_PIC = ALIVE_PIC if ALIVE_PIC else "https://te.legra.ph/file/29728c17f73d73bdc3da3.jpg"
  

CRAXY = "✯ ᶜᴿᴬˣʸ ˢᴾᴬᴹ ᴴᴱᴿᴱ ｡ﾟ･ ✯\n\n"
CRAXY += f"═══════════════════\n"
CRAXY += f"• **ᴘʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `3.10.1`\n"
CRAXY += f"• **ᴛᴇʟᴇᴛʜᴏɴ ᴠᴇʀsɪᴏɴ** : `{version.__version__}`\n"
CRAXY += f"• **ᴄʀᴀxʏʙᴏᴛ ᴠᴇʀsɪᴏɴ**  : `{CRAXYversion}`\n"
CRAXY += f"═══════════════════\n\n"   
...
async def alive(event):
  if event.sender_id in SUDO_USERS:
     await BOT0.send_file(event.chat_id,
        ...
        [
        Button.url("• ʀᴇᴘᴏ •", "https://github.com/CRAXY026/CRAXY_SPAM")
        ]
        ]
        )
...
async def restart(e):
    if e.sender_id in SUDO_USERS:
        text = "R ᴇ ʟ ᴏ ᴀ ᴅ ɪ ɴ ɢ ↪️.. Please Wait Until It Starts Again"
...
async def tb(event):
    if event.sender_id == OWNER_ID:
        ...
        if sudousers:
            ...
            newsudo = f"{target}"
        await ok.edit(f"**Added `{target}` ** as a sudo user ⚡ R ᴇ ꜱ ᴛ ᴀ ʀ ᴛ ɪ ɴ ɢ.. Please wait a minute...")
        heroku_var[CRAXY] = newsudo   
