...
from resources.data import RAID, REPLYRAID, CRAXYSPAM, BRTHSPAM
from CRAXYSPAM import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9, SUDO_USERS, OWNER_ID
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        CRAXY = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        ...
        if len(CRAXY) == 2:
            ...
            g = a.id
            if int(g) in CRAXY:
                text = f"I can't raid on @CRAXYMARRK's Owner"
                await e.reply(text)
            elif int(g) == OWNER_ID:
...
async def _(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in CRAXY:
                text = f" can't raid on @CRAXYMARRK's Owner."
                await e.reply(text)
...
async def _(event):
   ...
   if event.sender_id in SUDO_USERS:
         ...
         CRAXY = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(CRAXY) == 3:
             user = str(CRAXY[2])
             a = await event.client.get_entity(user)
             e = a.id
             if int(e) in CRAXYSPAM:
                    text = f"I can't raid on @CRAXYMARRK's Owner"
                    await event.reply(text)
             elif int(e) == OWNER_ID:
                text = f"This guy is a owner Of this Bots."
                await event.reply(text)
                 ...
                 counter = int(CRAXY[1])
                 sleeptimet = sleeptimem = float(CRAXY[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
                      async with event.client.action(event.chat_id, "typing"):
